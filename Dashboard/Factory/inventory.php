...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($whatAction === 'add_product') {
        ...
        $result = $conn->query("SELECT id FROM factory_product WHERE created_for = '$user_name' ORDER BY CAST(SUBSTRING(id, 5) AS UNSIGNED) DESC LIMIT 1 FOR UPDATE");
        ...
        // Fetch latest product ID current or previous year
        $result = $conn->query("SELECT stock_id FROM factory_stock WHERE created_for = '$user_name' ORDER BY CAST(SUBSTRING(stock_id, 5) AS UNSIGNED) DESC LIMIT 1 FOR UPDATE");
            ...
            if ($productResult && $productRow = $productResult->fetch_assoc()) {
                ...
                if (is_array($rawMaterialsArr)) {
                    foreach ($rawMaterialsArr as $rm) {
                        ...
                        $updateStmt->bind_param("dis", $usedQty, $rawMaterialId, $user_name);
                        $updateStmt->execute();
                        $updateStmt->close();
                    }
                }
...
<body>
    ...
    $rawMaterialSql = "SELECT id, material, cost FROM factory_raw_material WHERE created_for = '$user_name'";
    ...
    $addedbySql = "SELECT DISTINCT addedBy FROM factory_expenses ORDER BY addedBy";
    $addedbyResult = $conn->query($addedbySql);
    $added = [];
    if ($addedbyResult->num_rows > 0) {
        while ($row = $addedbyResult->fetch_assoc()) {
            $added[] = $row['addedBy'];
        }
    }
    ?>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="productSelect" required
                                ...
                                <?php
                                $result = $conn->query("SELECT * FROM factory_product WHERE created_for = '$user_name'");
                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        $stockSql = "SELECT quantity as prevstock, value as prevvalue, avg_new_sale as prevavg 
                                            FROM factory_stock 
                                            WHERE item_name = '$productName' AND created_for = '$user_name' 
        ...
        <div id="factory">
            <div class="container-fluid d-flex justify-content-between align-items-center">
                ...
                <div class="justify-content-end">
                    <a href="factory_dashboard.php?page=inventory&view=<?php echo isset($_GET['view']) && $_GET['view'] === 'all' ? 'none' : 'all'; ?>"
    ...
    <div class="col-md-12 card p-3 shadow-sm my-4 table-responsive">
        <div id="factory">
            ...
            <table id="Table" class="table table-bordered table-hover">
                ...
                <tbody>
                    <?php
                    $sql = "SELECT * FROM factory_product WHERE created_for = '$user_name' ORDER BY id DESC";
                    $result = $conn->query($sql);
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo "<tr>";
                        ...
                        echo "<tr><td colspan='" . ($hasDeletePermission ? 13 : 12) . "'>No product data found</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['deleteProduct']) && $hasDeletePermission) {
        ...
        $deleteSql = "DELETE FROM factory_product WHERE id = ? AND created_for = ?";
